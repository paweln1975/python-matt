...
phone book for. For each  queried, print the associated entry from your phone book
on a new line in the form name=phoneNumber; if an entry for  is not found, print
Not found instead.
...
on a single line. The first value is a friend's name, and the second value is a digit
phone number.
...
in the phone book; otherwise, print the full name and phone in the format name=phoneNumber.
...
def get_phone(phone_book: dict[str, str], name: str):
...
# Create a dictionary to store the phone book
    phone_book = {}
